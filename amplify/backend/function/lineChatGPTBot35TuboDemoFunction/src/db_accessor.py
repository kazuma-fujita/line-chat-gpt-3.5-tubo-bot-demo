...
# async def query_by_line_user_id(line_user_id: str, limit: int) -> list:
...
#     }
#     try:
#         # Call the query method of the DynamoDB client with the query parameters asynchronously
#         async with aioboto3.client('dynamodb') as dynamodb:
#             query_result = await dynamodb.query(**query_params)
...
# async def put_message(partition_key: str, uid: str, role: str, content: str, now: datetime) -> None:
...
#         },
#     }
#     # Try to put the item into the table using dynamodb client asynchronously
#     try:
#         async with aioboto3.client('dynamodb') as dynamodb:
#             await dynamodb.put_item(**options)
